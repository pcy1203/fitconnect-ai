...
class GeneralInterviewAnalysis(BaseModel):
    ...
    keywords: List[str] = Field(
        description="답변 전반에서 반복적으로 등장하는 주요 키워드",
        max_length=5,
        default_factory=list,
    )
    experiences: List[str] = Field(
        description="지원자가 중요하게 언급한 대표 경험과 성과",
        max_length=5,
        default_factory=list,
    )
    strengths: List[str] = Field(
        description="지원자의 핵심 역량과 행동적 강점",
        max_length=5,
        default_factory=list,
    )
    job_competencies: List[str] = Field(
        description="지원자의 직무 역량 (기술 직군의 경우에 한해 기술 스택)",
        max_length=5,
        default_factory=list,
    )
    working_styles: List[str] = Field(
        description="지원자의 업무 방식과 협업 스타일, 성장 가능성",
        max_length=5,
        default_factory=list,
    )
    technical_stacks: List[str] = Field(
        description="지원자의 기술 스택 (기술 직군에 한해) 혹은 활용 가능한 툴 (무관한 직무는 빈 값 가능)",
        max_length=10,
        default_factory=list,
    )
